...
`krakend.json` 是 KrakenD 的主要設定檔。本課程的檔案位於 repo 根目錄的 `krakend.json`，常見最小結構如下：
...
- repo 根目錄的 `krakend.json`
- 容器內的 `/etc/krakend/krakend.json`
...
- 本課程使用 Compose service name `mock-api` 作為 backend host。
